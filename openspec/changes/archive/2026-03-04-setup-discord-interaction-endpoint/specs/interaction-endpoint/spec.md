...
DiscordからのInteraction Endpoint URLの検証リクエスト（Type 1: PING）に正しく応答しなければならない。
...
すべての `/interactions` へのリクエストは、DiscordのPublic Keyを使用して署名検証を行わなければならない。
...
- **WHEN** 不正な署名、または欠損した署名ヘッダーを含むリクエストを受信する
- **THEN** システムは HTTP 401 Unauthorized を返さなければならない

### Requirement: Health Check Endpoint
システムの生存確認のためのエンドポイントを提供しなければならない。
