## Context
既存の `python-checks.yaml` は、プルリクエスト（PR）作成時に常に実行されますが、開発初期の Draft PR 段階では、Lint やテストがパスしないことが想定されるため、これらのチェックは Ready for Review になってから実行するのが効率的です。一方で、PR 説明欄の TODO リストの完遂状況は、作業の進捗を管理するために Draft 段階から可視化されるべきです。

## Goals / Non-Goals

**Goals:**
- 各品質チェックワークフローに Draft PR 時のスキップロジックを導入する。
- `check-todo.yml` のみを Draft PR 段階でも実行される唯一のワークフローとする。
...
- **Issue チェックのロジック**
  - `actions/github-script` を使用し、タイトルと本文の両方を対象に正規表現 `/#\d+/` で検索します。リファレンスの実装をそのまま踏襲します。

- **TODO チェックのロジック**
  - `grep -qE '^([[:space:]]*)?- \[ \]'` を使用して、未チェックの項目を検出します。こちらもリファレンスの実装を踏襲しつつ、`if` 条件を外して Draft でも動くようにします。
